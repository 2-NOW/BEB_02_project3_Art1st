...
contract Art1stToken is ERC20, Ownable {
    constructor() ERC20("Art1stToken", "AT") {
          // _mint(msg.sender, 100000000e18); -> 새로운 토큰이 발급될 때마다 _mint를 실행시키도록 하자.
    }
    ...
    function compensate(address to, uint amount) public onlyOwner returns (bool) {
        address server = _msgSender();
    ...
    function donate(address from, address to, uint amount) public onlyOwner returns (bool) {
        address server = _msgSender();
